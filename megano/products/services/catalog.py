...
class CatalogByCategoriesMixin:
    ...
    def simple_sort(cls, some_list: List, sort_type: str) -> Tuple:
        """
        метод сортирует входящую последовательность в соответствии с заданным типом и направлением сортировки
    ...
    @classmethod
    def get_full_data(cls, tag: str = '', search: str = '', slug: str = '') -> Tuple[QuerySet or List, List]:
        ...
        tags = cls.get_tags(some_goods=goods, main_tag=tag)
    ...
    def get_data_by_slug(cls, some_slug: str) -> QuerySet:
        ...
        return Product.objects.select_related('category', 'tags').filter(category__in=acceptable_categories).annotate(
            Count('product_comments'))

    @classmethod
    def get_data_by_tag(cls, some_tag: str) -> QuerySet:
        """метод возвращает список товаров по тэгу"""

        return Product.objects.select_related('category', 'tags').filter(
            tags__name__icontains=some_tag
        ).annotate(Count('product_comments'))
    ...
    def get_data_by_search_query(cls, some_search_query: str) -> QuerySet:
        ...
        return Product.objects.select_related('category', 'tags').filter(
            Q(name__icontains=some_search_query) |
            Q(category__name__icontains=some_search_query) |
            Q(tags__name__icontains=some_search_query)
        ).annotate(Count('product_comments'))
    ...
    def filtering_data(cls, some_goods: QuerySet, filter_data: Dict) -> QuerySet:
        ...
        some_goods = some_goods.filter(
            name__icontains=filter_data['f_title'],
            quantity__gte=filter_data['in_stock'],  # ??
            limited__icontains=filter_data['is_hot'],  # ??
        ).annotate(Count('product_comments'))
        print(f'big filter*** {some_goods}')  # ?
        if filter_data.get('tag', False):
            some_goods = some_goods.filter(
                tags__name=filter_data['tag'],
            )
    ...
    @classmethod
    def get_full_data_with_filters(cls, search_query: str or None, search_tag: str or None, slug: str,
                                   filter_data: Dict) -> Tuple:
        """
        ...
        :param filter_data: словарь со значениями фильтров для отображения карточек товаров  # ?
        ...
        tags = cls.get_tags(goods, search_tag)

        return goods, tags

    @classmethod
    def get_tags(cls, some_goods: QuerySet, main_tag: str = '') -> List:
        """метод возвращает тэги по списку товаров"""
        tags = []
    ...
    def sort_by_name(cls, some_list: List, direction: bool) -> List:
        ...
        """
        return sorted(some_list, key=lambda x: x.name, reverse=direction)  # ?? x

    @classmethod
    def sort_by_pop(cls, some_list: List, direction: bool) -> List:
        """
        метод сортировки товаров по количеству заказов
        :param some_list: исходный список товаров
        :param direction: направление сорировки; true - по возрастанию, false - по убыванию
        :return: отсортированный по наименованию список
        """
    ...
    def sort_by_price(cls, some_list: List, direction: bool) -> List:
        return sorted(some_list, key=lambda x: x.price, reverse=direction)  # ?

    @classmethod
    def sort_by_amount_of_comments(cls, some_list: List, direction: bool) -> List:
        """
        метод сортировки товаров по количеству комментариев
        :param some_list: исходный список товаров
        :param direction: направление сорировки; true - по возрастанию, false - по убыванию
        :return: отсортированный по количеству комментариев список
        """
        return sorted(some_list, key=lambda x: x.product_comments.count(), reverse=direction)

    @classmethod
    def sort_by_newness(cls, some_list: List, direction: bool) -> List:
        """
        метод сортировки товаров по новизне
        :param some_list: исходный список товаров
        :param direction: направление сорировки; true - по возрастанию, false - по убыванию
    ...
    def get_min_price(cls, some_list: List) -> int or float:
        ...
        :param some_list: список товаров
